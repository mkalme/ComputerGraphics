...
namespace PathTracer {
    public class Renderer {
        ...
        private Color CastRay(Ray ray, int depth, Shape ignoreShape = null) {
            TraceResults result = TraceRay(ray, ignoreShape);
            ...
            //Shading
            Shading shading = GetShadingColor(shape.GetNormal(point), point, ray, shape);
            ...
            if (shape.Properties.ReflectionIndex > 0 && depth < MAX_DEPTH) {
                ...
                color = CastRay(rr, depth + 1, shape).AddTint(color, shape.Properties.ReflectionIndex);
            }

            return color.AddColor(shading.SpecuralColor);
        }
        private TraceResults TraceRay(Ray ray, Shape ignoreShape = null) {
            bool ignore = ignoreShape != null;

            List<TraceResults> intersections = new List<TraceResults>();
            foreach (Shape shape in Scene.Shapes) {
                if (ignore) {
                    if (Object.ReferenceEquals(shape, ignoreShape)) {
                        ignore = false;
                        continue;
                    }
                }

                TraceResults results = shape.Intersect(ray);

                if (results.Intersects) {
                    intersections.Add(results);
                }
            }

            if (intersections.Count == 0) return new TraceResults();

            return intersections.OrderBy(p => p.Distance).First();
        }
        private TraceResults MarchRay(Ray ray, Shape ignoreShape = null)
        {
            ...
            for (int i = 0; i < MAX_STEPS; i++) {
                ...
                if (minD <= MIN_DIST) {
                    return new TraceResults(true, closestShape, point, d);
                } else if (minD >= 100) {
                    return results;
                }
            }
        ...
        //Shading
        private Shading GetShadingColor(Vec3 n, Vec3 p, Ray vr, Shape shape)
        {
            Vec3 v = vr.Point2 - vr.Origin;
            v.Normalize();

            Color color = shape.GetColor(p);
            ...
            foreach (LightSource light in Scene.LightSources) {
                ...
                TraceResults shadowResults = TraceRay(new Ray(p, light.Pos), shape);
